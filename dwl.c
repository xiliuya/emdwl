...
static struct wl_listener idle_inhibitor_create
    = { .notify = createidleinhibitor };
static struct wl_listener idle_inhibitor_destroy
...
static struct wl_listener new_layer_shell_surface
...
static struct wl_listener session_lock_mgr_destroy
    = { .notify = destroysessionmgr };
...
unset_fullscreen:
  ...
  wl_list_for_each (w, &clients,
                    link) if (w != c && w->isfullscreen && m == w->mon
                              && (w->tags & c->tags)) setfullscreen (w, 0);
}
  ...
  if (cursor_mode == CurPressed && !seat->drag)
    {
      if ((type = toplevel_from_wlr_surface (
               seat->pointer_state.focused_surface, &w, &l))
          >= 0)
        {
          c = w;
